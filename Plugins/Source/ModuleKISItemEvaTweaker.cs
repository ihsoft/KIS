...
namespace KIS
{
    ...
    {
        [KSPField]
        public float walkSpeed = -1; //Default : 0.8f
        [KSPField]
        public float runSpeed = -1; // Default : 2.2f
        [KSPField]
        public float ladderSpeed = -1; // Default : 0.6f
        [KSPField]
        public float swimSpeed = -1; // Default : 0.8f
        [KSPField]
        public float maxJumpForce = -1; // Default : ?
        ...
        {
            KerbalEVA kerbalEva = item.inventory.part.GetComponent<KerbalEVA>();
            if (walkSpeed != -1) kerbalEva.walkSpeed = orgWalkSpeed;
            if (runSpeed != -1) kerbalEva.runSpeed = orgRunSpeed;
            if (ladderSpeed != -1) kerbalEva.ladderClimbSpeed = orgLadderSpeed;
            if (swimSpeed != -1) kerbalEva.swimSpeed = orgSwimSpeed;
            if (maxJumpForce != -1) kerbalEva.maxJumpForce = orgMaxJumpForce;
        }
